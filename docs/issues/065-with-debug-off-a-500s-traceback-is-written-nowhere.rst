...
:Resolution: (pending) -- ``LOGGING`` in ``common_settings.py`` with the reasoning beside it, the explicit quiet in ``test_settings.py``, and ``kasvimuseo/tests/test_settings_logging.py``. Nothing in ``ansible/`` changed and the status of 051 is untouched; the mail half is 066, filed with this and still open.
