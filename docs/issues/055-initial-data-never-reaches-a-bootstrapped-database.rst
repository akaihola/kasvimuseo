...
:Resolution: d6cdf05 -- the fixture moves to ``kasvimuseo/fixtures/``, ``dev/kasvimuseo``'s two stale comments are corrected, and ``test_fixtures_initial_data.py`` pins where it lives. No change to ``0022``, which stays correct and idempotent.
