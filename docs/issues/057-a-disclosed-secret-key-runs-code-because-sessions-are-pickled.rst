...
:Resolution: 5b34eb4 -- ``SESSION_SERIALIZER`` in ``common_settings.py`` with the reason beside it, and ``kasvimuseo/tests/test_settings_session_serializer.py``. The server-side acts of 049, 050 and 051 are untouched and still open; this only removes what a disclosed key is worth.
