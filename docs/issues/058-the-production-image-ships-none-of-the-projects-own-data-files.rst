...
:Resolution: 4b34240 -- ``COPY MANIFEST.in`` into the build context in ``Dockerfile``, plus a ``RUN`` block that fails the build if any of the six files it carries is not readable in the installed package. ``ylaneenkasvit/locale/`` and ``kasvimuseo/locale/`` were missing too, and are fixed by the same line; the assertion covers them.
