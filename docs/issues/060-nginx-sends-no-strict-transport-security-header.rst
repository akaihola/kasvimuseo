...
:Resolution: (none yet) -- stage 1 is committed, in COMMIT: the ``add_header`` in ``ansible/templates/nginx-site.conf.j2``, the argument in that template's Jinja comment, and the deploy step and its consequence in ``README.rst`` under "The security maintenance window". That is everything this repository can do to *stage 1*, and the header is real on the next ``install.yaml`` run and not before. This issue closes with stage 2 -- ``max-age=31536000`` in the same directive -- which is one line, is deliberately not in this commit, and has conditions written below
