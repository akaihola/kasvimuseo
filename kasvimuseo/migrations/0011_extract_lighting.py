...
from ..models import LIGHTINGS


class Migration(DataMigration):

    def forwards(self, orm):
        """Extract lightings from substrate field"""
        for species in orm.Species.objects.all():
            parts = re.split(ur', *', species.substrate, 1)
            for value, label in LIGHTINGS:
                if parts[0].lower() == label.lower():
                    species.lighting = value
                    species.substrate = u''.join(parts[1:])
                    species.save()


    def backwards(self, orm):
        """Join lighting into the substrate field"""
        lightings_dict = dict(LIGHTINGS)
        for species in orm.Species.objects.filter(lighting__isnull=False):
            label = lightings_dict[species.lighting]
            species.substrate = u'%s, %s' % (label, species.substrate)
            species.lighting = None
            species.save()
